...
| 2026-07-18 (S0-04) | **Dev + staging infrastructure rebaselined onto the existing Hostinger VPS (Ubuntu 24.04) + Coolify + Docker Compose — replacing the unconfirmed managed-cloud / Terraform baseline. Managed cloud + Terraform are DEFERRED to production, not removed.** Owner decision (authoritative): use the existing shared VPS (already running Coolify) for dev + staging; ARA Tasks stays **isolated** on it and must not alter other projects. Deploy bundle added under `deploy/vps/` — a Coolify-compatible Compose **override** layered on the root `docker-compose.yml`, reusing the S0-03 images (no build logic duplicated): PG18+PostGIS, Redis, and MinIO are **self-hosted and private** (no public ports); api/web/operator are exposed only through Coolify's reverse proxy on **separate domains**. **Original `S0-04` ticket wording, preserved verbatim before rebaselining** — Scope: *"Terraform base infra in-region: managed PG+PostGIS, Redis, private bucket, secret manager"*; Done when: *"terraform apply provisions dev env"*. | The managed-cloud/in-region assumption was never confirmed and blocked no code, while the owner has a working VPS + Coolify **today**. Self-hosting PG18+PostGIS / Redis / MinIO on the VPS unblocks a real dev/staging environment now and keeps the **locked app stack, DB technology, modular-monolith, and two-plane security boundary unchanged** (tenant vs operator = separate Coolify services + domains). Managed KSA/GCC cloud + Terraform stay on the roadmap as the **production** evolution path, gated on the still-open hosting-region / PDPL-cert decision (Tech Stack §16). Data services publish no public ports (Security Design §7/§12); secrets live in Coolify's encrypted env, never in Git (CLAUDE.md rule 9). Docs updated: Task Breakdown `S0-04`, Roadmap Sprint 0, Tech Stack §1/§11/§16, System Design §9, System Architecture §8. |
| 2026-07-18 (S0-05) | **CI/CD = one GitHub Actions pipeline; deployment reaches Coolify only via a webhook the pipeline calls after all gates pass — Coolify auto-deploy-on-push is disabled.** One workflow (`.github/workflows/ci.yml`) with `validate` + `deploy` jobs. Triggers: `pull_request` (validate only — no `environment:`, no secrets, no deploy), `push`→`main` (validate → auto `staging` deploy), `workflow_dispatch` (validate → manual `development`/`staging`). Two **GitHub Environments** (`development`, `staging`) hold env-scoped **secrets** (`COOLIFY_WEBHOOK`, `COOLIFY_TOKEN`) + non-secret **vars** (`WEB_URL`/`API_URL`/`OPERATOR_URL`/`API_EXPECTED_STATUS`). All actions **pinned to immutable commit SHAs**; least-priv `permissions`; validation cancels stale runs while deploys **serialize per env and never cancel in-progress**. Migration stage is an **honest `S0-09`-pending gate** (visible notice, exit 0) that becomes real by adding `db:migrate:ci` — no redesign. `actionlint` is used as a **pinned, checksum-verified runtime tool**, not a package dependency. | GitHub Actions is already the locked CI/CD (Tech Stack §11) and Coolify the locked dev/staging host (`S0-04`) — this is the implementation, not a stack change, so **no new major dependency** (CLAUDE.md rule 1). Routing every deploy through the pipeline keeps `lint→typecheck→test→build→migrate` un-bypassable (disabling Coolify's own auto-deploy is what enforces that). `pull_request` (never `pull_request_target`) + environment-gated secrets + the `if != pull_request` deploy guard mean **a PR can never obtain deployment secrets** (Security Design §11/§12). SHAs pin supply chain; per-env serialization protects live deploys; secrets are never printed (only a `scheme://host` redaction). Remote acceptance (creating the Environments + Coolify resources) is **pending UI/credential access** and explicitly not claimed as proven. |
| 2026-07-18 (S0-05) | **Reformatted the S0-03/S0-04 Compose YAML to satisfy `pnpm format:check`, and excluded harness `.claude/` from Prettier.** `docker-compose.yml` + `deploy/vps/docker-compose.staging.yml` used double quotes against the repo's `singleQuote: true` Prettier config; the new `validate` job's `format:check` step surfaced this **pre-existing** failure (prior tickets only ran `turbo build/lint/typecheck`). `prettier --write` fixed it; `.claude/` added to `.prettierignore`. | A CI gate that is red on its first run against already-committed files is not "done". The reformat is **whitespace/quote-only and semantically inert** — `git diff --ignore-all-space` shows only `"`→`'` + comment spacing, and the rendered `docker compose config` for both stacks is **byte-identical before/after** (verified by sha256), with privacy checks and the `!override`/`!reset` merge tags unchanged. Excluding `.claude/` from Prettier keeps local `format:check` honest without touching harness files (they are gitignored and absent from CI checkouts anyway). No behavior change to the S0-03/S0-04 stacks. |
...
|---|---|---|
| **Hosting region (production)** — GCP Dammam (KSA) vs AWS Bahrain (me-south-1) / UAE (me-central-1) | Nothing in code; **dev + staging now run on the VPS (S0-04)**; must be settled **before production** | Confirm the region carries the PDPL/compliance certs the target clients require. Most important open item (Tech Stack §16); managed cloud + Terraform are deferred onto this decision. |
