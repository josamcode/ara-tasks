...
|---|---|---|---|
| **S0-05** | Remote acceptance (staging deploy). | — | Repo side done + verified. Create the `development`/`staging` GitHub Environments + secrets/vars and the Coolify deploy resources/webhooks per [`.github/CI_CD_SETUP.md`](../../.github/CI_CD_SETUP.md), then prove PR-validation + `main`→staging deploy + Coolify completion + web/operator `2xx` + api expected status. |
